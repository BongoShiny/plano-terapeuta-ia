...
14. PROIBIÇÃO ABSOLUTA: NUNCA mencione "ansiedade", "estresse", "depressão", "saúde mental", "emocional" ou qualquer termo psicológico em NENHUM campo do plano terapêutico. Isso é um plano de terapia CORPORAL/MUSCULAR — foque exclusivamente em aspectos físicos, musculares e posturais.
11. PROIBIÇÃO ABSOLUTA DE SUPOSIÇÕES: Use EXCLUSIVAMENTE as informações fornecidas nos DADOS DO PACIENTE acima. NUNCA invente, suponha, deduza ou reformule informações que não foram explicitamente informadas. Se um campo diz "Não informado", NÃO tente adivinhar ou criar informações sobre ele. Se o histórico diz "Não informado", NÃO invente um histórico. Cada frase do plano DEVE ser verificável nos dados fornecidos. Inventar informações médicas é EXTREMAMENTE GRAVE e causa desconforto ao paciente.
12. CAUSAS PROVÁVEIS — REGRA CRÍTICA: No campo "Causas Prováveis" dos dados acima estão listadas as ÚNICAS causas que podem ser mencionadas no plano. COPIE-AS EXATAMENTE como estão escritas. NUNCA adicione causas que NÃO estejam na lista. NUNCA substitua por sinônimos. Se a única causa listada for "Não sei ao certo", NÃO mencione causas em NENHUM campo do plano — simplesmente omita qualquer referência a causas. NUNCA escreva frases como "as causas exatas das dores não são claras", "as causas não são conhecidas", "a origem da dor é incerta" ou qualquer variação similar. Exemplos de ERROS GRAVES: se as causas dizem "Hérnia de Disco, Bruxismo", NUNCA escreva "postura no trabalho", "sedentarismo", "acidente" ou qualquer outra causa que NÃO esteja explicitamente listada.
13. CONDIÇÕES PREEXISTENTES: Mencione SOMENTE as condições listadas no campo "Condições Preexistentes". NÃO invente condições adicionais.

JSON a retornar (preencha todos os campos com conteúdo real e personalizado para ${data.nome}):

{
  "resumo_queixas": "Escreva UM parágrafo de no máximo 3 frases, acolhedor e completo, sobre as queixas de ${data.nome}. REGRAS OBRIGATÓRIAS PARA ESTE CAMPO: (A) Áreas afetadas — use SOMENTE estas: ${(data.areas_afetadas || []).join(", ")}. Reescreva removendo barras: 'Pescoço / Cervical' → 'pescoço', 'Lombar / Coluna Lombar' → 'coluna lombar', 'Cabeça / Crânio' → 'cabeça', 'Mandíbula / ATM' → 'mandíbula'. (B) Causas prováveis — COPIE EXATAMENTE E SOMENTE ESTAS: ${(data.causas_provaveis || []).join(", ") || "NÃO INFORMADAS — não mencione causas"}. Se as causas não foram informadas ou se a única causa é "Não sei ao certo", NÃO mencione causas no texto — simplesmente OMITA qualquer referência a causas. NUNCA escreva "as causas não são claras", "as causas exatas não são conhecidas" ou qualquer frase similar. Se foram informadas causas específicas, use os MESMOS TERMOS sem reformular. (C) Condições preexistentes — mencione SOMENTE ESTAS se relevante: ${(data.condicoes_preexistentes || []).join(", ") || "Nenhuma"}. (D) NUNCA mencione tempo de dor. (E) NUNCA invente causas, acidentes, traumas ou eventos. Termine com ponto final.",
