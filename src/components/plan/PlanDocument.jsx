...
function PosturalColumns({ text }) {
  ...
  const BulletList = ({ sentences }) => (
    <ul style={{ margin: 0, paddingLeft: 0, listStyle: "none" }}>
      {sentences.map((s, i) => (
        <li key={i} style={{ display: "flex", gap: 6, fontSize: 13, marginBottom: 6, lineHeight: 1.7, color: "#222" }}>
  ...
  return (
    ...
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: "#1B3A4B", marginBottom: 6 }}>Na vista frontal (plano coronal)</div>
      <BulletList sentences={frontalSentences} />
    </div>
    <div style={{ width: 1, background: "#D1C4B0", flexShrink: 0 }} />
    <div style={{ flex: 1 }}>
      <div style={{ fontSize: 13, fontWeight: 700, color: "#1B3A4B", marginBottom: 6 }}>Na vista lateral (plano sagital)</div>
      <BulletList sentences={lateralSentences} />
    </div>
    </div>
  );
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginTop: 10 }}>
            {safeArray(planData.fotos_camera_termal).map((url, i) => (
              <div key={i} style={{ borderRadius: 6, overflow: "hidden", border: "1px solid #E5E7EB" }}>
                <img src={url} alt={`Termal ${i + 1}`} style={{ width: "100%", display: "block", objectFit: "cover", height: 320 }} />
              </div>
            ))}
          </div>
        </PageWrapper>
